"""Tests for ImageObject.list_supported_images and filter_out_appledouble_files.
...
from detectorist.image_object import APPLEDOUBLE_MAGIC, ImageObject
...
def test_sidecar_excluded_when_sibling_present(tmp_path):
    ...
    assert ImageObject.list_supported_images(str(tmp_path)) == ["IMG_0001.jpg"]
...
def test_sidecar_excluded_via_magic_number_without_sibling(tmp_path):
    ...
    assert ImageObject.list_supported_images(str(tmp_path)) == []
...
def test_dot_underscore_file_kept_without_sibling_or_magic_number(tmp_path):
    ...
    assert ImageObject.list_supported_images(str(tmp_path)) == ["._IMG_0003.jpg"]
...
def test_unrelated_dotfile_excluded_by_extension_filter(tmp_path):
    ...
    assert ImageObject.list_supported_images(str(tmp_path)) == ["IMG_0004.jpg"]
...
def test_mixed_case_extensions_still_matched(tmp_path):
    ...
    assert ImageObject.list_supported_images(str(tmp_path)) == ["IMG_0005.HIF", "IMG_0006.Jpg"]
...
def test_filter_out_appledouble_files_drops_sidecar_with_sibling(tmp_path):
    ...
    kept = ImageObject.filter_out_appledouble_files([str(original), str(sidecar)])
...
def test_filter_out_appledouble_files_checks_magic_number_without_sibling(tmp_path):
    ...
    assert ImageObject.filter_out_appledouble_files([str(sidecar)]) == []
...
def test_filter_out_appledouble_files_does_not_match_across_directories(tmp_path):
    ...
    kept = ImageObject.filter_out_appledouble_files([str(unrelated_original), str(lookalike_sidecar)])
